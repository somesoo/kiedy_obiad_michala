...
module.exports = function createSeasonalModule(deps) {
  ...
  function candiesOnBoard() {
    return db.prepare('SELECT id, tile FROM sl_candies WHERE board = ? AND collected_by IS NULL')
      .all(getSeason().id);
  }
  ...
  function payload(playerId) {
    ...
    if (ev.trick_or_treat) {
      out.trick_or_treat = { tiles: ev.trick_or_treat.tiles, weekdays: ev.trick_or_treat.weekdays, active: trickActive(todayWaw()) };
    }
    if (ev.candy) {
      ...
      out.candy = {
        tiles: candiesOnBoard().filter(c => c.tile > 0).map(c => c.tile),
  ...
  return {
    initSchema, resolveLanding, emitFor, payload, ensureCandySpawn,
