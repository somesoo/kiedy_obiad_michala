...
module.exports = function createSeasonalModule(deps) {
  ...
  // Domyślne stawki drzwi — plik planszy może je nadpisać (treat_points, candy_points,
  // trick_coins). Noc Duchów ma drzwi na sześciu polach codziennie, więc bierze mniejsze.
  // Rejestr zapisuje FAKTYCZNE kwoty, więc zmiana stawek nie psuje cofania starych ruchów.
  ...
  // Bez `weekdays` drzwi są otwarte codziennie (weekend i tak stoi, bo nie ma rzutów).
  function trickActive(day) {
    const ev = events();
    if (!ev || !ev.trick_or_treat) return false;
    const days = ev.trick_or_treat.weekdays;
    return !days || days.includes(weekdayOf(day));
  }

  function trickStakes(t) {
    return {
      treat: t.treat_points || TREAT_POINTS,
      candy: t.candy_points || TREAT_CANDY_POINTS,
      trick: t.trick_coins || TRICK_COINS,
    };
  }
  ...
  function blockedTiles() {
    ...
    // Rozstaje: cukierek leżałby tylko na jednym z dwóch numerów tego samego miejsca, więc
    // połowa stających tam graczy przechodziłaby po nim bez podniesienia.
    for (const pair of s.shared || []) pair.forEach(p => out.add(p));
    return out;
  }
  ...
  function resolveLanding({ playerId, landedAbs, day, turnRef }) {
    ...
    if (ev.trick_or_treat && ev.trick_or_treat.tiles.includes(tile) && trickActive(day)) {
      ...
      if (r < 0.5) {
        ...
        const pts = withCandy ? trickStakes(ev.trick_or_treat).candy : trickStakes(ev.trick_or_treat).treat;
        return {
          abs: landedAbs,
      ...
      if (r < 0.75) {
        const eggCoins = trickStakes(ev.trick_or_treat).trick;
        return {
          abs: landedAbs,
          result: { kind: 'trick_egg', tile, coins: -eggCoins },
          apply: ref => {
            ledger(playerId, 'trick_egg', { coins: -eggCoins, tile, ref, day });
            slLogActivity(playerId, 'season_event', `🥚 Cukierek albo psikus → psikus! Zgniłe jajo: -${eggCoins} coins`, turnRef);
          }
        };
      }
  ...
  function payload(playerId) {
    ...
    if (ev.trick_or_treat) {
      out.trick_or_treat = {
        tiles: ev.trick_or_treat.tiles, weekdays: ev.trick_or_treat.weekdays || null,
        stakes: trickStakes(ev.trick_or_treat),
