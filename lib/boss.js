...
module.exports = function createBossModule(deps) {
  ...
  function startCoopBossEvent(coop) {
    ...
    // ── BOSS SEZONOWY ──
    // Pierwsza NOWA edycja po `start_from` to boss z pliku sezonu, a jej termin to zawsze
    // `attack_at` (np. noc Halloween). Nie ucinamy bossa, który akurat trwa — gracze mogli
    // już w niego wpłacić; edycje trwają kilka dni roboczych, więc przy start_from na ~dwa
    // tygodnie przed atakiem zwykły boss zdąży się skończyć. HP rośnie z liczbą dni
    // roboczych do ataku (tyle, ile ma ekipa na walkę), z sufitem ×1.5 i podłogą ×0.5.
    const sp = getSpecialBoss();
    const now = Date.now();
    if (sp && now >= sp.startMs && now < sp.attackMs
        && !db.prepare('SELECT 1 FROM sl_coop WHERE special = ?').get(sp.key)) {
      let days = 0;
      while (days < 30 && addBusinessDaysMs(now, days + 1) <= sp.attackMs) days++;
      // Mniej niż dwa dni robocze na walkę to już nie walka — wtedy zwykły boss.
      if (days >= 2) {
        const scale = Math.min(1.5, Math.max(0.5, days / Math.max(1, Number(coop.time_limit_days))));
        name = sp.name;
        maxHp = Math.round(Number(coop.threshold) * SL_BOSS_HP_MULTIPLIER * sp.hp_factor * scale);
  ...
  // Zapowiedź bossa sezonowego dla UI: od `announce_from` do końca doby ataku, dopóki
  // jego edycja się nie zakończyła. `active` = to on jest teraz bossem.
  function slSpecialBossTeaser(coop) {
    const sp = getSpecialBoss();
    if (!sp) return null;
    const now = Date.now();
    if (now < sp.announceMs || now > sp.attackMs + 12 * 3600 * 1000) return null;
    const row = db.prepare('SELECT status FROM sl_coop WHERE special = ? ORDER BY cycle DESC LIMIT 1').get(sp.key);
    if (row && row.status === 'completed') return null;
    return {
      name: sp.name, emoji: sp.emoji,
      attack_at: new Date(sp.attackMs).toISOString(),
      start_from: new Date(sp.startMs).toISOString(),
      active: !!(coop && coop.special === sp.key)
    };
  }
