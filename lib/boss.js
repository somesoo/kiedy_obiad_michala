...
module.exports = function createBossModule(deps) {
  ...
  // ── STAŁE ──────────────────────────────────────────────────────────────────
  // Bazowe HP bossa. Zeszło z 3500, bo 3500 było NIE DO ZDJĘCIA: przy 10 grających
  // kość daje przez 5 dni roboczych ~1575 obrażeń (3,5 oczka × 3 × 3 rzuty × 10 osób
  // × 5 dni), a wpłaty realnie dokładają ~500 — razem ~2100. Boss, którego nie da się
  // ubić, nie jest trudny, tylko zepsuty: ekipa nigdy nie widziała nagrody, za to co
  // pięć dni obrywała karą.
  const SL_COOP_THRESHOLD = parseInt(process.env.SNAKES_COOP_THRESHOLD, 10) || 2000;
  ...
  function slApplyDiceDamage(playerId, rolls, moveId) {
  ...
  function slCoopPayload(meId) {
    ...
    return {
      ...
      pace: {
        ...
        last_day: daily.length ? daily[0] : null,
        daily
      },
