...
module.exports = function createBossModule(deps) {
  ...
  // Boss sezonowy (np. Dynia Zagłady na Halloween) — opcjonalny, z pliku planszy.
  const getSpecialBoss = deps.getSpecialBoss || (() => null);
  ...
  function initSchema() {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS snakes.sl_boss_payouts (
        ...
        cycle      INTEGER NOT NULL,
        player_id  INTEGER,
        kind       TEXT NOT NULL,   -- 'contrib' | 'fighter' | 'podium' | 'milestone' | 'penalty' | 'season_refund'
    ...
    // Klucz bossa sezonowego ('halloween-2026'), gdy ta edycja nim jest; NULL = zwykły boss.
    ensureColumn('sl_coop', 'special', 'TEXT');
  ...
  function startCoopBossEvent(coop) {
    let name = SL_BOSS_NAMES[Math.floor(Math.random() * SL_BOSS_NAMES.length)];
    let maxHp = Math.round(Number(coop.threshold) * SL_BOSS_HP_MULTIPLIER);
    let deadlineMs = addBusinessDaysMs(Date.now(), Number(coop.time_limit_days));
    let special = null;

    // ── BOSS SEZONOWY ──
    // Sezon z `special_boss` ma JEDNEGO bossa: nazwa z pliku, termin zawsze `attack_at`
    // (np. noc Halloween), a HP = stawka × dni robocze do ataku — tyle, ile ekipa ma
    // czasu na walkę. Stawka jest ustawiona trochę powyżej tempa, które ekipa robi przy
    // sporych wpłatach, więc bez wpłat się go nie ubije. Klucz edycji (`special`) pilnuje,
    // żeby w tym samym roku nie wrócił po wygranej/przegranej.
    const sp = getSpecialBoss();
    const now = Date.now();
    if (sp && now < sp.attackMs && !db.prepare('SELECT 1 FROM sl_coop WHERE special = ?').get(sp.key)) {
      let days = 0;
      while (days < 60 && addBusinessDaysMs(now, days + 1) <= sp.attackMs) days++;
      // Mniej niż dwa dni robocze na walkę to już nie walka — wtedy zwykły boss.
      if (days >= 2) {
        name = sp.name;
        maxHp = sp.hp_per_workday * days;
        deadlineMs = sp.attackMs;
        special = sp.key;
      }
    }

    db.prepare(`
      UPDATE sl_coop SET boss_name = ?, boss_max_hp = ?, boss_hp = ?, special = ?,
                         boss_started_at = CURRENT_TIMESTAMP, boss_deadline_at = datetime(?, 'unixepoch')
      WHERE cycle = ?
    `).run(name, maxHp, maxHp, special, Math.floor(deadlineMs / 1000), coop.cycle);
    if (special) {
      const when = new Date(sp.attackMs).toLocaleString('pl-PL', { timeZone: 'Europe/Warsaw', weekday: 'long', day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' });
      slEmit('coop_completed', () => `${sp.emoji} **${sp.name} nadciąga!** ${maxHp} HP — zaatakuje: **${when}**. Rzucajcie i wpłacajcie, zanim będzie za późno.\n${snakesUrl()}`);
    }
    return { started: true, cycle: Number(coop.cycle), boss_name: name, boss_max_hp: maxHp, deadline_ms: deadlineMs, special };
  }

  // Informacja o bossie sezonowym dla UI (odliczanie do ataku). Tylko dopóki jego edycja
  // trwa — po wygranej albo przegranej znika. `active` = to on jest teraz bossem.
  function slSpecialBossTeaser(coop) {
    const sp = getSpecialBoss();
    if (!sp || !coop || coop.special !== sp.key || coop.status === 'completed') return null;
    return { name: sp.name, emoji: sp.emoji, attack_at: new Date(sp.attackMs).toISOString(), active: true };
  }

  // Wystawia bossa sezonowego OD RAZU, gdy sezon go ma, a jeszcze się nie pojawił:
  // przy starcie serwera, po przełączeniu sezonu w panelu i w każdym tiku schedulera.
  // Trwającą zwykłą walkę zamykamy BEZ nagród i BEZ kar (nikt jej nie przegrał ani nie
  // wygrał — ustąpiła miejsca), a coins wpłacone w nią wracają w całości. Zwrot idzie
  // przez rejestr wypłat (kind 'season_refund'), więc ścieżki cofania go widzą.
  function slEnsureSpecialBoss() {
    const sp = getSpecialBoss();
    if (!sp || !slBossEnabled() || Date.now() >= sp.attackMs) return null;
    if (db.prepare('SELECT 1 FROM sl_coop WHERE special = ?').get(sp.key)) return null;
    return transaction(() => {
      const coop = slCurrentCoop();
      if (coop.special === sp.key) return null;
      const paid = db.prepare(`
        SELECT player_id, SUM(amount) AS coins FROM sl_coop_contributions
        WHERE cycle = ? AND source = 'coins' GROUP BY player_id
      `).all(coop.cycle);
      for (const p of paid) {
        slRecordPayout(coop.cycle, p.player_id, 'season_refund', 0, Number(p.coins));
        slLogActivity(p.player_id, 'boss_reward',
          `↩️ ${coop.boss_name} ustąpił miejsca bossowi sezonu (${sp.name}) — zwrot wpłaty: +${p.coins} coins`,
          slBossActivityRef(coop.cycle, 'season_refund'));
      }
      db.prepare(`UPDATE sl_coop SET status = 'completed', completed_at = CURRENT_TIMESTAMP WHERE cycle = ?`).run(coop.cycle);
      const next = slCoopInsertCycle(Number(coop.cycle) + 1, Number(coop.threshold), Number(coop.time_limit_days));
      console.log(`Snakes/Boss: boss sezonu ${sp.name} wystawiony (edycja #${next.cycle}, ${next.boss_max_hp} HP); zwrot wpłat: ${paid.length} graczy`);
      return { cycle: Number(next.cycle), refunded: paid.length };
    });
  }
  ...
  // Klucz bloku w dzienniku dla rozliczenia, które dotyka wielu graczy naraz (kamień
  // milowy, wygrana, kara). Każdy gracz dostaje własny wpis — podświetlenie „to ja" liczy
  // się z `player_id` — ale przy karze to kilkanaście identycznych wierszy, więc front
  // zwija je w jeden blok z rzędem awatarów. Ten sam `ref` co bloki tury (slNewTurnRef),
  // tylko z innym prefiksem. Nikt nie cofa wpisów po `ref`, więc to wyłącznie widok.
  function slBossActivityRef(cycle, kind) {
    return `boss:${cycle}:${kind}`;
  }
    ...
    for (const pct of SL_COOP_MILESTONES) {
      ...
      for (const c of contributors) {
        ...
        slLogActivity(c.player_id, 'boss_reward',
          ...
          slBossActivityRef(coop.cycle, `milestone:${pct}`));
      }
  ...
  function slFinishBossEvent(coop, defeated) {
    ...
    for (const p of payouts) {
      ...
      slLogActivity(p.player_id, 'boss_reward',
        ...
        slBossActivityRef(coop.cycle, 'victory'));
    }
    ...
    if (!defeated) {
      ...
      for (const p of allPlayers) {
        ...
        slLogActivity(p.player_id, 'boss_reward',
          ...
          slBossActivityRef(coop.cycle, 'penalty'));
      }
    }
  ...
  function slCoopPayload(meId) {
    ...
    return {
      ...
      special_boss: slSpecialBossTeaser(coop),
      boss: coop.boss_name ? {
        name: coop.boss_name,
        hp,
  ...
  return {
    ...
    // walka
    slApplyDiceDamage, slFinishBossEvent, resolveCoopBossEvent, slResolveBossTimeout, slEnsureSpecialBoss,
