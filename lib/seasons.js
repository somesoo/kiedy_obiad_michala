...
//   snakes   — [[z, na], …], na < z
//   bonuses  — [[pole, pkt], …]
//   shared   — [[a, b], …]: ROZSTAJE — dwa numery pól stojące w JEDNYM miejscu (tor, który
//              przecina sam siebie). Oba numery muszą mieć identyczny punkt w `path`.
//              Dla zbijania to jedno pole: kto stanie na a, zbija tego z b. Rozstaje nie
//              mogą być startem ani polem łącznika i nie mają zdarzeń; mogą być bonusem, ale
//              wtedy tym SAMYM na obu numerach — to jedno miejsce, więc płaci tak samo.
//   testing  — true = sezon jeszcze w testach, nikt na nim naprawdę nie grał: liczbę pól
//              i numerację wolno zmieniać bez migracji. Po premierze → false (albo usunąć).
//   lap_points — opcjonalne: punkty (i coins) za ukończone okrążenie; brak = domyślne
//              SL_POINTS_PER_LAP z server.js. Mniejsza plansza = częstsze okrążenia, więc
//              sezon z krótką drogą może chcieć mniejszej premii.
...
//   shop_at    — [x, y] (środek, w kratkach): chatka garderoby narysowana na planszy.
//                Bez niej garderoba otwiera się przyciskiem w pasku nad planszą.
...
//     trick_or_treat:  { tiles: [pola], weekdays?: [1–5], hide_bonuses?, treat_points?,
//                      candy_points?, trick_coins? } — 50/50 cukierek (pkt / 🍬) albo psikus.
//                      `weekdays` (1 = poniedziałek) ogranicza drzwi do tych dni; brak =
//                      drzwi otwarte CODZIENNIE. `hide_bonuses: true` = w dni drzwi pola
//                      bonusowe nie działają (drzwi ZASTĘPUJĄ bonusy). Stawki: pkt za cukierek
//                      (domyślnie 15), pkt za cukierek z 🍬 (5), coins za zgniłe jajo (15).
...
//   special_boss — JEDYNY boss sezonu, np. Dynia Zagłady na Halloween:
//                { name, emoji, attack_at: 'MM-DD HH', hp_per_workday }. Pojawia się od razu,
//                gdy sezon jest aktywny (także w trakcie trwającej walki — ta zamyka się bez
//                nagród i kar, a wpłacone coins wracają), i trwa aż do `attack_at` (czas
//                Warszawy, BEZ roku — sezon działa co roku). HP = hp_per_workday × dni
//                robocze do ataku.
...
function validate(b) {
  ...
  // Rozstaje: pary numerów w jednym miejscu. Sprawdzane przed odstępami, bo to jedyny
  // dozwolony wyjątek od „żadne dwa pola nie stoją za blisko".
  const sharedPair = new Map(); // pole -> drugi numer tego samego miejsca
  if (b.shared != null) {
    const size = b.path.length;
    if (!Array.isArray(b.shared)) errors.push('"shared" musi być listą par [a, b]');
    else for (const pair of b.shared) {
      const [x, y] = Array.isArray(pair) ? pair : [];
      const what = `rozstaje ${JSON.stringify(pair)}`;
      if (!isInt(x) || !isInt(y) || x <= 0 || y <= 0 || x >= size || y >= size || x === y) {
        errors.push(`${what}: dwa różne pola 1–${size - 1}`); continue;
      }
      if (sharedPair.has(x) || sharedPair.has(y)) { errors.push(`${what}: pole jest już w innych rozstajach`); continue; }
      const px = b.path[x], py = b.path[y];
      if (!Array.isArray(px) || !Array.isArray(py) || px[0] !== py[0] || px[1] !== py[1]) {
        errors.push(`${what}: oba numery muszą mieć identyczny punkt w "path"`); continue;
      }
      sharedPair.set(x, y); sharedPair.set(y, x);
    }
  }
  if (b.testing != null && typeof b.testing !== 'boolean') errors.push('"testing" musi być true albo false');
  if (free && !errors.length) {
    for (let i = 0; i < b.path.length; i++) {
      for (let j = i + 1; j < b.path.length; j++) {
        if (sharedPair.get(i) === j) continue;
  ...
  if (b.special_boss != null) {
    const sb = b.special_boss;
    const ok = sb && typeof sb.name === 'string' && sb.name.trim()
      && (sb.emoji == null || (typeof sb.emoji === 'string' && sb.emoji.length <= 8))
      && typeof sb.attack_at === 'string' && /^\d{2}-\d{2} \d{2}$/.test(sb.attack_at)
      && isInt(sb.hp_per_workday) && sb.hp_per_workday > 0;
    if (!ok) errors.push('"special_boss": potrzebne name, attack_at ("MM-DD HH") i hp_per_workday > 0');
  }
  if (b.lap_points != null && !(isInt(b.lap_points) && b.lap_points >= 0)) errors.push('"lap_points" musi być liczbą całkowitą ≥ 0');
  ...
  if (b.shop_at != null && !(Array.isArray(b.shop_at) && isNum(b.shop_at[0]) && isNum(b.shop_at[1]))) {
    errors.push('"shop_at" musi być punktem [x, y]');
  }
  ...
  if (ev != null) {
    if (typeof ev !== 'object' || Array.isArray(ev)) errors.push('"events" musi być obiektem');
      ...
      if (ev.trick_or_treat != null) {
        ...
        const okDays = t && (t.weekdays == null || (Array.isArray(t.weekdays) && t.weekdays.length
          && t.weekdays.every(d => isInt(d) && d >= 1 && d <= 5)));
        const okStake = (v) => v == null || (isInt(v) && v > 0);
        if (!t || !Array.isArray(t.tiles) || !t.tiles.length || !okDays) {
          errors.push('events.trick_or_treat: potrzebne tiles [pola]; weekdays (opcjonalne) to lista 1–5');
        } else if (!okStake(t.treat_points) || !okStake(t.candy_points) || !okStake(t.trick_coins)) {
          errors.push('events.trick_or_treat: treat_points, candy_points i trick_coins muszą być liczbami całkowitymi > 0');
  ...
  // Rozstaje to jedno miejsce, więc oba numery muszą działać tak samo. Łącznik z rozstajów
  // prowadziłby w dwa różne miejsca zależnie od numeru, a zdarzenie (drzwi, kocioł) albo
  // cukierek leżałby na „połowie" pola — tego gracz na planszy nie odróżni.
  const bonusAt = new Map((Array.isArray(bonuses) ? bonuses : []).filter(Array.isArray).map(([p, v]) => [p, v]));
  const evAt = new Set();
  if (ev && typeof ev === 'object') {
    if (ev.cauldron && Array.isArray(ev.cauldron.drop)) { ev.cauldron.drop.forEach(p => evAt.add(p)); evAt.add(ev.cauldron.ladle); }
    if (ev.trick_or_treat && Array.isArray(ev.trick_or_treat.tiles)) ev.trick_or_treat.tiles.forEach(p => evAt.add(p));
  }
  for (const [x, y] of sharedPair) {
    if (x > y) continue;
    const what = `rozstaje [${x}, ${y}]`;
    for (const t of [x, y]) {
      const on = occupied.get(t);
      if (on && !on.startsWith('bonus')) errors.push(`${what}: na polu ${t} stoi ${on} — rozstaje mogą być tylko zwykłym polem albo bonusem`);
      if (evAt.has(t)) errors.push(`${what}: na polu ${t} jest zdarzenie sezonowe`);
    }
    if (bonusAt.get(x) !== bonusAt.get(y)) errors.push(`${what}: bonus musi być taki sam na obu numerach (${bonusAt.get(x) || 0} ≠ ${bonusAt.get(y) || 0})`);
  }
  return errors;
}
...
function normalize(id, b) {
  ...
  return {
    ...
    lap_points: b.lap_points != null ? b.lap_points : null, // null = domyślne z server.js
    // Rozstaje: pary numerów w jednym miejscu (logika zbijania + rysunek jednego pola).
    shared: (b.shared || []).map(([x, y]) => [x, y]),
    testing: !!b.testing,
    special_boss: b.special_boss ? { ...b.special_boss, emoji: b.special_boss.emoji || '👹' } : null,
...
function list() {
  return [...all().values()].map(s => ({ id: s.id, name: s.name, size: s.size, cols: s.cols, rows: s.rows, theme: s.theme, testing: s.testing }));
}
