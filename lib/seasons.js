...
//     trick_or_treat:  { tiles: [pola], weekdays: [1–5], hide_bonuses? } — drzwi czynne
//                      tylko w te dni (1 = poniedziałek); 50/50 cukierek (pkt / 🍬) albo
//                      psikus. `hide_bonuses: true` = w te same dni pola bonusowe nie działają
//                      (drzwi ZASTĘPUJĄ bonusy, a nie dokładają się do nich)
...
//   special_boss — boss sezonowy, np. na Halloween:
//                { name, emoji, announce_from: 'MM-DD', start_from: 'MM-DD',
//                  attack_at: 'MM-DD HH', hp_factor }. Daty BEZ roku (czas Warszawy) —
//                sezon odpala się tak samo co roku. Od `announce_from` UI zapowiada bossa,
//                pierwsza NOWA edycja po `start_from` jest nim, a jej termin to `attack_at`.
...
function validate(b) {
  ...
  if (b.special_boss != null) {
    ...
    const md = v => typeof v === 'string' && /^\d{2}-\d{2}$/.test(v);
    const ok = sb && typeof sb.name === 'string' && sb.name.trim() && (sb.emoji == null || (typeof sb.emoji === 'string' && sb.emoji.length <= 8))
      && md(sb.announce_from) && md(sb.start_from)
      && typeof sb.attack_at === 'string' && /^\d{2}-\d{2} \d{2}$/.test(sb.attack_at)
      && isNum(sb.hp_factor) && sb.hp_factor >= 0.5 && sb.hp_factor <= 3
      && sb.announce_from <= sb.start_from && sb.start_from < sb.attack_at.slice(0, 5);
    if (!ok) errors.push('"special_boss": potrzebne name, announce_from ≤ start_from < attack_at (MM-DD, attack_at "MM-DD HH") i hp_factor 0.5–3');
  }
